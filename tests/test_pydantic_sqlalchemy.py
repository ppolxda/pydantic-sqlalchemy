...
from pydantic_sqlalchemy import sqlalchemy_select_to_pydantic, sqlalchemy_to_pydantic
...
def test_select_query_schema() -> None:
    ...
    sql = select(func.sum(User.id), func.sum(User.id).label("sum_uid")).outerjoin(
        Address
    )
    schema = sqlalchemy_select_to_pydantic("PydanticUserWithAddresses", sql)
    assert schema.schema() == {
        "title": "PydanticUserWithAddresses",
        "type": "object",
        "properties": {"sum_uid": {"title": "Sum Uid", "type": "integer"},},
        "required": ["sum_uid"],
    }
